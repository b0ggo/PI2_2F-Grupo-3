...
import { getLote, putLote } from "../../services/api.js";
import "../CadastroLotes/CadastrarLotes.css";
...
function loteParaForm(lote) {
  return {
    tipo: lote.tipo || "Suínos",
    nome: lote.nome || "",
    quantidade: lote.quantidade != null ? String(lote.quantidade) : "",
    doentes: lote.doentes != null ? String(lote.doentes) : "",
    vacinados: lote.vacinados != null ? String(lote.vacinados) : "",
    mortes: lote.mortes != null ? String(lote.mortes) : "",
...
export default function EditarLote() {
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      await putLote(id, payload);
      navigate(ROUTES.CONSULTAR, { state: { atualizado: true, aba: "lotes" } });
    } catch (error) {
      const msg =
    ...
    return (
      <div className="cadastro-page">
        <div className="cadastro-card">
          <Header titulo="Editar Lote" voltarPara={ROUTES.CONSULTAR} />
          ...
          <div className="cadastro-actions">
            ...
            >
              {submitting ? (
                ...
                <>
                  <Icon d={ICON_CHECK} size={16} color="#fff" /> Salvar Alterações
                </>
              )}
            </button>
          </div>
